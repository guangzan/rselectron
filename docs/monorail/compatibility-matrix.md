...
- Evidence / 证据: `.github/workflows/ci.yml` runs format/Rslint/typecheck/build/docs, Rstest (including packed-tarball, generation, config-restart, HMR/shutdown coverage), and Playwright Electron smoke across Ubuntu/Windows/macOS x64+arm64 runners with Electron 41 and 43; failure uploads retain rstest/playwright/fixture artifacts. Exact Prettier/`@rslint/core` pins are asserted by `tests/ci-workflow.test.ts`. / `.github/workflows/ci.yml` 在 Ubuntu/Windows/macOS x64+arm64 runner 上对 Electron 41 与 43 运行 format/Rslint/typecheck/build/docs、Rstest（含 packed-tarball、generation、config-restart、HMR/shutdown）与 Playwright Electron 冒烟；失败时上传 rstest/playwright/fixture 产物。`tests/ci-workflow.test.ts` 断言精确的 Prettier/`@rslint/core` 钉扎。
