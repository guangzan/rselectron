...
The frozen Electron support window of a particular Rselectron release: a fixed floor of Electron 28 (the first ESM-capable major) and the three stable majors current at release time as the rolling top. The snapshot remains fixed for that Rselectron version. Each major records Node and Chromium version strings (from its first stable release) and ESM capability, used for derived compiler targets. Versions outside the snapshot are rejected with a structured unsupported-version error.

**Rsbuild tested window**  
The frozen range of `@rsbuild/core` versions a particular Rselectron release was tested against: the minor line of the version pinned in that release's development dependency set (for example, tested against `2.1.7` gives a window of `>=2.1.0 <2.2.0`). Patch-level updates within the tested minor line are presumed safe and produce no diagnostic. A project-local `@rsbuild/core` outside the window produces a warn-only structured diagnostic; it is never a hard error, because `@rsbuild/core` is a required project peer that the application owns as its build tool. This contrasts with the Electron support snapshot, whose out-of-window versions are hard-rejected because Rselectron derives compiler targets from per-major runtime metadata.
