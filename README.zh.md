...
当前 beta：**1.0.0-beta.0**（发布后可用 `npm install rselectron@1.0.0-beta.0`，或从打包 tarball 安装）。
...
`rselectron/node` 声明 `RSELECTRON_RENDERER_URL` 以及 `?asset` / `?asset&asarUnpack` / `?modulePath` / `?nodeWorker` / `*.wasm?loader` / `*.node` 等模块形态。
Core 与 CLI 仍是私有实现包。
